...
sudo ip link set dev veth2 netns c2
sudo ip netns exec c2 ip link set dev veth2 name eth0 address 02:42:c0:a8:00:20
sudo ip netns exec c2 ip addr add dev eth0 10.0.0.20/24
sudo ip netns exec c2 ip link set dev eth0 up

sudo ip netns exec overns ip neighbor add 10.0.0.10 lladdr 02:42:c0:a8:00:10 dev vxlan1
sudo ip netns exec overns bridge fdb add 02:42:c0:a8:00:10 dev vxlan1 self dst 192.168.50.10 vni 42 port 4789
